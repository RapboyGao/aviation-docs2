<template>
	<div>
		...
		<el-checkbox
			v-model="filterByFavorites"
			label="只显示Favorites"
			size="large"
		/>
		<div v-if="sortBySize">
			<a href="/aviation-docs/mp3-hardest/001-050.mp3" target="_blank">
				000-050
			</a>
			<a href="/aviation-docs/mp3-hardest/051-100.mp3" target="_blank">
				051-100
			</a>
			<a href="/aviation-docs/mp3-hardest/101-150.mp3" target="_blank">
				101-150
			</a>
			<a href="/aviation-docs/mp3-hardest/151-200.mp3" target="_blank">
				151-200
			</a>
			<a href="/aviation-docs/mp3-hardest/201-250.mp3" target="_blank">
				201-250
			</a>
			<a href="/aviation-docs/mp3-hardest/251-300.mp3" target="_blank">
				251-300
			</a>
		</div>
	</div>
	<ElDivider></ElDivider>
	<div
		v-for="(item, index) in sortedFiltered"
		:key="item.index"
		class="mt-50"
		:id="`pepec-${index}`"
	>
		<ElCard>
			<template #header>
				<div>
					<div class="flex space-between">
						...
						<el-checkbox
							v-model="favorites[item.index]"
							label="Favorite"
							size="large"
						/>
						<el-button @click="playAudio(item)" text type="primary"
							>播放</el-button
						>
					</div>
				</div>
			...
			<template #default>
				<Ques>
					<div>
						<span v-if="item.mismatch || item.specialReading"
							>官方原文：
						</span>
...
function getSourceName(item: ContentProtocol): string {
	...
	return `/aviation-docs/mp3/${fileName}.mp3`;
}
...
const sorted = computed(() => {
	if (sortBySize.value) {
		return pepec.sort(
			($0, $1) => getActualLength($1) - getActualLength($0)
		);
	} else {
...
const sortedFiltered = computed(() =>
	sorted.value.filter(($0) =>
		filterByFavorites.value ? favorites.value[$0.index] : true
	)
);
</script>
